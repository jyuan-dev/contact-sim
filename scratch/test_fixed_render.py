...
def render_mask_fixed(env, state, target='block', width=224, height=224):
    ...
    # Save original COG and colors
    orig_cog = raw_env.block.center_of_gravity
    orig_colors = {}
    for shape in raw_env.space.shapes:
        orig_colors[shape] = shape.color

    # CRITICAL FIX: Set COG to (0, 0) so setting angle does not shift the center position!
...
def main():
    ...
    overlay[block_mask > 127] = [0, 0, 255] # Red highlight
    overlay[agent_mask > 127] = [0, 255, 0] # Green highlight
    
    out_dir = '/home/jyuan/.gemini/antigravity-ide/brain/84ca7224-7271-41ba-9541-8c03fd85f0e9'
    Image.fromarray(overlay).save(os.path.join(out_dir, 'overlay_fixed_debug.png'))
    print("Saved overlay image to:", os.path.join(out_dir, 'overlay_fixed_debug.png'))
