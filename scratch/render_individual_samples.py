...
def main():
    ...
    with h5py.File(h5_path, 'r') as f:
        for step in steps_to_test:
            ...
            goal_mask = render_mask_fixed(env, state, target='goal', width=224, height=224)
            
            Image.fromarray(orig_img).save(os.path.join(out_dir, f'sample_orig_{step}.png'))
            Image.fromarray(block_mask).save(os.path.join(out_dir, f'sample_block_mask_{step}.png'))
            Image.fromarray(agent_mask).save(os.path.join(out_dir, f'sample_agent_mask_{step}.png'))
            Image.fromarray(goal_mask).save(os.path.join(out_dir, f'sample_goal_mask_{step}.png'))
            print(f"Saved step {step} sample images.")
